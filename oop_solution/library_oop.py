...
class Book:
    ...
    def is_me(self, check_id):
        """Check if ID is the same as target ID"""
        if self.id == check_id:
            return True
        else:
            return False
...
class Libaray:
    ...
    def __init__(self):
        """Initialize book and member collection"""
        self.book_list = []
        self.member_list = []

    def add_book(self,id, title, author, copies):
        """register a new library book"""
    ...
    def __return_book(self, target_id):
        """Process a returning book. return True if suceed. otherwise False"""
        for book_obj in self.book_list:
            if book_obj.is_me(target_id):
